...
export interface AppSettings {
    ...
    // Windows only, DEFAULT ON (opt-out). An always-on WH_KEYBOARD_LL hook
    ...
    // registration lets a chord's key reach the foreground app. Unset ⟹ enabled;
    // only an explicit `false` disables it. The trade-off is that an
    // always-present low-level keyboard hook is more visible to EDR/AV than one
    // that exists only during stealth-typing sessions — hence the opt-out.
